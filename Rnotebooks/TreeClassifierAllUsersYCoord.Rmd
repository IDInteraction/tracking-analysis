---
title: "TreeClassificationAllUsersYCoord"
author: "Aitor Apaolaza, Robert Haines"
date: "16/12/2015"
...
*Authors: Aitor Apaolaza, Robert Haines*

##Initialisation and loading data
```{r, echo=FALSE}
#Initialisation functions, no need to echo the functions to the document
source("TreeClassifierCodeResources.R")

projectFolderPath <- "/home/docker/data/"
annotatedCSVFolder <- paste(projectFolderPath, "attention/", sep="")
trackingCSVFolder <- paste(projectFolderPath, "tracking/", sep="")
...
In this setup the regular Y coordinate is used, instead of the relative Y coordinate.
...
```{r, echo=FALSE}
  ...
  for(participantIndex in participantCodeList){
    ...
    loadAndCreateFeatureDF(outputPath,
                           annotatedCSVFolder,
                           trackingCSVFolder)
...
From a total of `r nrow(dataclass)` rows, 2/3 of them (`r nrow(trainsetclass)`) will be used for training, and 1/3 for testing (`r nrow(testsetclass)`).

##Training the tree classifier
```{r, echo=FALSE}
#I only use the following package to be able to convert the formula back to text
library(formula.tools)
formula = attentionName ~ boxHeight + boxRotation + boxArea + boxWidth + widthHeightRatio + boxYcoord
```
...
Error metrics based on prediction of attention.
