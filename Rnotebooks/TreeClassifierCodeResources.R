...
loadCSVFilesForParticipant <-function(annotatedCSVFolder,
                                      trackingCSVFolder){
  annotatedCSVFilename <<- paste(annotatedCSVFolder,participantCode,"-timings.csv",sep="")
  trackingCSVFilename <<- paste(trackingCSVFolder,participantCode,"_video.csv",sep="")  
}

loadAndCreateFeatureDF <- function(projectFolderPath,
                                   annotatedCSVFolder,
                                   trackingCSVFolder){
  loadCSVFilesForParticipant(annotatedCSVFolder,
                             trackingCSVFolder)
  combineCSVsToDF()
  createFeatureDF()
...
compareFeatures <- function(){
  ...
  folderPath = paste(projectFolderPath,"/Resources/explorationPlots/",sep="")
  filename = paste(folderPath,"features","_",participantCode,"_",gsub(":", ";", Sys.time()),".png",sep="")
  width=3
  height=1.5
