...
projectFolderPath <- "/home/aitor/Dropbox/UoM/IDInteractionCode/"

experimentName <- "dual_screen_free_experiment"

videoQuality <- "high_quality"# lowres or high_quality

trackingMode <- "front_eyes_only" # front_full_face or front_eyes_only

attentionSourceFolder <- paste(projectFolderPath,"resources/",experimentName,"/",videoQuality,"/attention/",sep="")
trackingSourceFolder <- paste(projectFolderPath,"resources/",experimentName,"/",videoQuality,"/",trackingMode,"/",sep="")
...
compareFeatures <- function(){
  ...
  explorationPlotsFolder <- paste(projectFolderPath,"results/exploration_plots/",experimentName,"/",videoQuality,"/",trackingMode,"/",sep="")
  
  filename = paste(explorationPlotsFolder,"features","_",participantCode,"_",gsub(":", ";", Sys.time()),".png",sep="")
  width=3
  height=1.5
